...
//index.php?tarz_topic_id=1&tarz_turkish=0&tarz_swedish=1&tarz_english=1&tarz_font_size=48
//topicSelect, turkishChecked, swedishChecked, englishChecked, fontSize, searchTerm

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    echo '<p id="mesaj"></p><script>';
    foreach ($_POST as $key => $value) {
        // Eğer boolean bir değer ise, true/false olarak kaydet
        if (in_array($key, [ 'turkishChecked', 'swedishChecked', 'englishChecked' ])) {
            $boolValue = $value == 1 ? 'true' : 'false';
            echo "sessionStorage.setItem('" . htmlspecialchars($key) . "', " . $boolValue . ");";
        } else {
            echo "sessionStorage.setItem('" . htmlspecialchars($key) . "', '" . htmlspecialchars($value) . "');";
        }
    }



 /*?>document.getElementById("mesaj").innerText = sessionStorage.getItem('searchResults');<?php */

    //echo '</script>';echo "php:".$_POST['searchResults']."php";
}
...
<div class="topic-container">
    ...
    <select id="topicSelect" name="topik" onclick="updateLanguages()">
   ...
   <a href="javascript:void(0)" id="logoutButton" title="Çıkış" class="button-link">Logout</a>
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const topicSelect = sessionStorage.getItem("topicSelect") || "0";
    ...
    document.getElementById("topicSelect").value = topicSelect;
    ...
    for (let i = 0; i < arabicTexts.length; i++) {
        arabicTexts[i].style.fontSize = fontSize + "px";
    }
    document.getElementById("sizeValue").innerText = fontSize + "px";
...
function changeVerse() {
    ...
    // Arapça metinlerin yazı boyutunu güncelle
    const arabicTexts = document.getElementsByClassName("arabic");
    for (let i = 0; i < arabicTexts.length; i++) {
        arabicTexts[i].style.fontSize = size + "px";
    }

    // Yazı boyutunu ekranda göster
    document.getElementById("sizeValue").innerText = size + "px";
}

    function updateLanguages() {
console.log("updateLanguages çağrıldı");

    const turkishChecked = document.getElementById("turkish").checked;
    ...
    const topicSelect = topicSelectElement ? topicSelectElement.value : "0"; // Eğer öğe yoksa varsayılan değer "0"
    ...
    sessionStorage.setItem("topicSelect", topicSelect);

    const turkishTexts = document.getElementsByClassName("turkish");
    const swedishTexts = document.getElementsByClassName("swedish");
    const englishTexts = document.getElementsByClassName("english");

    // Dil metinlerini görünür hale getir
    for (let i = 0; i < turkishTexts.length; i++) {
        turkishTexts[i].style.display = turkishChecked ? "inline" : "none";
    }
    for (let i = 0; i < swedishTexts.length; i++) {
...
document.getElementById("logoutButton").addEventListener("click", function (e) {
    e.preventDefault(); // Varsayılan davranışı engelle (sayfa yönlendirmesi)

    // sessionStorage'daki verileri al
    const topicSelect = sessionStorage.getItem("topicSelect") || "0";
    const turkishChecked = sessionStorage.getItem("turkishChecked") === "true" ? 1 : 0;
    const swedishChecked = sessionStorage.getItem("swedishChecked") === "true" ? 1 : 0;
    const englishChecked = sessionStorage.getItem("englishChecked") === "true" ? 1 : 0;
    const fontSize = sessionStorage.getItem("fontSize") || 32;
    const searchTerm = sessionStorage.getItem("searchTerm") || null;
    const searchResults = sessionStorage.getItem("searchResults") || null;

    // Verileri POST yöntemiyle logout.php'ye gönder
    fetch("logout.php", {
        method: "POST",
        headers: {
    ...
    .then(response => response.json()) // JSON formatında cevap bekliyoruz
    .then(data => {
